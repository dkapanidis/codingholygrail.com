...
function PostsList({ posts, topic }: Props) {
  return (
    <div className="pt-4">
      ...
      <div className="flex flex-col gap-4">{posts.map(post => <PostRow key={post.meta.slug} post={post} />)}</div>
    </div>
  )
}
      ...
      <a className="flex py-2 hover:bg-gray-100 rounded translate transform group cursor-pointer">
        <article className="space-x-4 flex ">
          ...
          <div className="flex flex-col">
            <div className="">
              <h2 className="text-lg font-semibold tracking-wider text-gray-600">{post.meta.title}</h2>
            </div>
            <div>
